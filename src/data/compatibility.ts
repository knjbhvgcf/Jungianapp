...
> = {
  ...
  sister: { label: 'Same Hero / Heroine', charge: 'mixed', order: 1 },
  mirror: { label: 'Hero / Heroine and Parent swapped', charge: 'mixed', order: 2 },
  parent: { label: 'Speaks your Parent', charge: 'steady', order: 3 },
  child: { label: 'Speaks your Eternal Child', charge: 'mixed', order: 4 },
  anima: { label: 'Inferior (Anima / Animus) pull', charge: 'charged', order: 5 },
  opposing: { label: 'Opposing Nemesis', charge: 'charged', order: 6 },
  senex: { label: 'Critic', charge: 'charged', order: 7 },
  trickster: { label: 'Trickster', charge: 'mixed', order: 8 },
  demon: { label: 'Demon', charge: 'charged', order: 9 },
}
...
function copyFor(me: PersonalityType, them: PersonalityType, kind: CompatKind): {
  ...
  switch (kind) {
    ...
    case 'sister':
      return {
        note: `The ${title} leads with your same Hero / Heroine, ${theirHero}, and cares with ${theirParent} instead of ${myParent}, so that the aim is shared and the stewardship is not, which is a kinship close enough to breed both relief and rivalry.`,
        detail: `You will recognise the ${fnName(myHero)} at once and argue about how people and problems ought to be looked after, their Parent, ${fnName(theirParent)}, being not a lesser version of your own but the other valid Parent for your Hero / Heroine; rivalry shows itself when one of you treats the other’s care as an amateur performance of a task you privately believe you were born to do.`,
      }
    case 'mirror':
      return {
        note: `The ${title} leads with ${theirHero}, which is your Parent, and supports with ${theirParent}, which is your Hero / Heroine, so that each of you lives as identity what the other uses to foster.`,
    ...
    case 'parent':
      return {
        ...
        detail: `You may feel useful, impressed, or talked down to, and they may feel taken seriously or treated as a tool of your Hero / Heroine, their Parent being ${fnName(theirParent)}; the pairing is easy for projects, and the watch is the urge to keep them in the advisor’s chair instead of letting them be a person with an aim of their own, which your Parent function, left unexamined, will not think to ask.`,
      }
    case 'child':
      return {
        note: `The ${title} leads with ${fnName(theirHero)}, which is your Eternal Child, so that play is easy and being taken equally seriously is not, a sweetness that can restore you and a slight that can quietly accumulate.`,
        detail: `They can restore you, and you can infantilize them, their Parent being ${fnName(theirParent)}, which is adult in a way you may not expect if you meet only their Hero / Heroine; if they meet only your Eternal Child, they will not know when you are actually in charge, and both misreadings are kinds of fondness that eventually ask to be corrected.`,
      }
    case 'anima':
      return {
        note: `The ${title} leads with ${fnName(theirHero)}, which is your Inferior (Anima / Animus), so that fascination, irritation, and projection travel together here, as they do wherever the unconscious has borrowed a human face.`,
        detail: `The Inferior (Anima / Animus) often wears this face, and you may want them to carry ${theirHero} for you and then resent that they are merely human at it, they supporting with ${fnName(theirParent)}; the work is to relate to them rather than to the missing piece they appear to be, and to practise your own Inferior in small doses so that the relationship is not your only classroom, a classroom which, overloaded, becomes a theatre of salvation and blame.`,
      }
    case 'opposing':
      return {
        note: `The ${title} leads with ${fnName(theirHero)}, the other attitude of your Hero / Heroine, so that arguments arise which sound like you turned inside out, a quarrel with the peculiar heat of a dispute inside one house.`,
        detail: `They are not your opposite so much as your ${myHero} facing the other way, and debates can clarify or never end, their Parent being ${fnName(theirParent)}; if you hear only a block, you miss the defense, for sometimes they are protecting an aim your Hero / Heroine is steamrolling, and it is worth naming the shared function before you name the fight, lest you spend years opposing what is already, in another attitude, your own.`,
      }
    case 'senex':
      return {
        ...
        detail: `The Critic is the Parent after kindness has left the building, which is a projection until it is not, their Parent being ${fnName(theirParent)}; you can use them as a boundary you will not set, or dismiss their competence because it arrives without nurture, and the question is whether they are attacking or whether you are hearing your own withheld no, which has been waiting for a mouth.`,
      }
    case 'trickster':
      return {
        note: `The ${title} leads with ${fnName(theirHero)}, the shadow of your Eternal Child, so that humour, loopholes, and binds gather here, and what feels like play may also be an escape neither of you has agreed to name.`,
