...
> = {
  hero: {
    ...
    watch: 'When the hero inflates, the other functions are treated as stupid or slow, and fatigue, contempt, and the resolve to “do it oneself” become the ordinary weather of the day; you may also grow brittle about being seen as this function, so that a joke at its expense lands not as humour but as an attack upon the self.',
    practice: 'Name, this week, one task that the hero ought not to run, and either hand it to the parent or allow it to remain unfinished, for competence that can share the stage is a sturdier thing than competence that must occupy it entirely, as if the rest of the psyche were a delay.',
    relating: 'People meet your hero as though it were your real self, and they will praise it, compete with it, or hide from it accordingly; it is worth noticing whom you secretly rank as serious according to whether they speak this function’s language, and whom you quietly set aside because they do not.',
  },
  parent: {
    work: 'This is how you foster, advise, and take responsibility, the supporting process that steadies the hero and is often what other people receive from you as care: the extra hour, the clarifying question, the standard you hold so that they need not, which, used well, is stewardship, and used automatically is a post you never applied for and cannot quite resign.',
  ...
  child: {
    work: 'This is play, relief, and improvisation, younger than the hero, charming, creative, and not fully accountable, a rest and not a government, and in love and friendship it is often the unguarded spark people fall for, precisely because it has not yet learned to govern itself and therefore has not yet learned to lie with professional skill.',
    watch: 'Inflation here looks like stubborn innocence, grand plans without follow-through, or a need for others to parent you, and under shame the child hides while the trickster, which is its shadow, begins to tie knots that will later be called accidents.',
    practice: 'Give the child a bounded playground — an hour, a sketch, a game with an end — and do not put it in charge of rent, surgery, or other people’s feelings, nor starve it either, for a starved child does not disappear but becomes a saboteur of the very order that refused it a room.',
  ...
  anima: {
    work: 'The inferior function is the door to the unconscious, at once aspirational and embarrassing, and you often meet it first in other people as longing, irritation, or idealization; Jung’s anima and animus are not costumes one puts on for effect, but the living other within, of which the inferior function is one of the usual faces, half desired and half feared.',
  ...
  opposing: {
    work: 'This is the opposite attitude of the hero, which argues, blocks, and says “yes, but,” and which can also defend you when the hero’s way is being steamrolled — a necessary no, spoken in the same function turned the other way; Beebe’s opposing personality is often the inner adversary that sounds almost like you, and is therefore the harder to recognise as other.',
    watch: 'Chronic oppositional spin undermines your own stated aim, or picks fights in the hero’s language turned inside out, and you may call it honesty or rigor when it is only a refusal to be moved, a pride that would rather be unconvinced than changed.',
    practice: 'When you hear “yes, but,” ask which aim is being protected, for sometimes the opposing function has a real objection and sometimes it is only pride, and it is the parent function, not the hero, that is fit to referee between them.',
    relating: 'People who lead with this function can feel like walking arguments, and the work is to notice the similarity: they are not your opposite so much as your hero facing the other way, which is why the quarrel has the peculiar heat of a family dispute.',
  },
  senex: {
    work: 'This is the shadow of the parent, which limits, criticizes, and says no, and which, used well, is a necessary boundary, the freeze that stops a bad rescue, and used poorly is bitterness and the verdict that you should have known; the senex, or witch, is the parent after it has given up on kindness and retained only the law.',
    ...
    practice: 'When the critic speaks, write its sentence, and then write the parent’s version of the same concern, keeping the limit and dropping the scorn; if you cannot, step away before you parent from the senex, for a boundary offered in contempt is not a boundary but a wound with a timetable.',
